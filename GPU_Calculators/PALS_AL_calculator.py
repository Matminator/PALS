...
class PALS_AL_ensemble(Calculator):
    implemented_properties = ['energy', 'energies', 'forces', 'stress', 'stresses']
    
    def __init__(self, model_paths, f_sigma_stop,
                 device = 'cpu', store_all = False, dir_name = None,
                 logo = True,
                 *args, **kwargs):
        """
        ...
        self.f_sigma_stop = f_sigma_stop
        self.stop_flag = False # For stopping calculations whan f_sigma > f_sigma_stop 
    ...
    def calculate(self, atoms=None, properties=None, system_changes=all_changes):
        ...
        f_sigma_max = np.max( np.sqrt(force_variance) )
        
        if f_sigma_max >= self.f_sigma_stop:
            self.stop_flag = True
...
#             raise 'Store all set to False'
    ...
    def print_logo(self):
        ...
        print('PALS AL ensamble version:', 1.1, '\n',flush = True)
